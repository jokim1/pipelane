...
function handleReviewPass(cwd: string, parsed: ParsedOperatorArgs): void {
  ...
  const evidence = buildAndPersistReviewPassEvidence(context, { gateId, message });
...
): { id: string; path: string; message: string } {
  try {
    ...
    return {
      id: persisted.id,
      path: evidencePath,
      message: renderReviewPassReport(persisted, options.gateId, evidencePath),
    };
  } catch (error) {
    if (!isReviewPassIndependentAiEvidenceError(error)) throw error;
    const record = buildReviewAcceptanceRecord({
      repoRoot: context.repoRoot,
      commonDir: context.commonDir,
      config: context.config,
      gateId: options.gateId,
    ...
    const persisted = appendReviewAcceptanceRecord(context.commonDir, context.config, record);
    const evidencePath = reviewAcceptanceStatePath(context.commonDir, context.config);
    return {
      id: persisted.id,
      path: evidencePath,
      message: renderReviewAcceptancePassReport(persisted, evidencePath),
    };
  }
}

function isReviewPassIndependentAiEvidenceError(error: unknown): boolean {
  return error instanceof Error && error.message.startsWith('review pass cannot attest independent AI gate ');
}

interface ReviewPassTarget {
  gateId: string;
  message: string;
...
function resolveReviewPassTarget(options: {
  repoRoot: string;
  commonDir: string;
  config: WorkflowConfig;
  gateId: string;
...
export function buildReviewPassRecord(options: {
  ...
  const { gateId, message, base, gate, worktreeStatus } = resolveReviewPassTarget(options);
...
export function buildReviewAcceptanceRecord(options: {
  repoRoot: string;
  commonDir: string;
  config: WorkflowConfig;
  gateId: string;
  ...
  const { gateId, message, currentBranch, currentSha, worktreeStatus, gate } = resolveReviewPassTarget(options);
  const recordedAt = nowIso();
  return {
    id: `review-acceptance-${new Date(recordedAt).toISOString().replace(/[-:.TZ]/g, '').slice(0, 14)}-${crypto.randomUUID().slice(0, 8)}`,
    gateId,
    gateDefinitionHash: reviewGateDefinitionHash(gate),
    acceptabilityClass: isIndependentAiReviewGate(gate) ? 'external-review' : 'manual-review',
    policyVersion: options.config.reviewGates?.policyVersion ?? 1,
    ...
    source: 'review-pass',
    reason: message,
    reasonHash: reviewAcceptanceReasonHash(message),
    recordedAt,
    signature: undefined,
  };
}
...
function hydrateReviewSetupEntryFromSavedGate(
  ...
  return {
    ...
    timeoutMs: savedGate.timeoutMs ?? entry.timeoutMs,
...
function formatReviewPrepareProgress(): string {
  return 'pipelane review: preparing gates';
}
...
function renderReviewRunReport(record: ReviewRunRecord, evidencePath: string, artifactRoot?: string): string {
  ...
  if (record.gates.length === 0) {
    ...
    for (const gate of presentation.gates) {
      const sourceGate = record.gates.find((entry) => entry.gateId === gate.gateId);
      const marker = sourceGate?.outcome === 'timeout' ? 'TIMEOUT' : gate.status.toUpperCase();
...
function renderReviewAcceptancePassReport(record: ReviewAcceptanceRecord, evidencePath: string): string {
  return [
    'Pipelane review pass',
    'Status: attested',
    `Evidence: ${evidencePath}`,
    ...
    '',
    'Next: rerun /pipelane review or continue the workflow to consume the signed acceptance.',
  ].join('\n');
}
