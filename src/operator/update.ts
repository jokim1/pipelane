...
import { LEGACY_REVIEW_GATES_POLICY_VERSION, REVIEW_GATES_POLICY_VERSION } from './review-gate-policy.ts';
import { homeClaudeDir, homeCodexDir, loadWorkflowConfig, pipelaneHomeDir, readJsonFile, resolveRepoRoot, runCommandCapture, writeJsonFile } from './state.ts';
...
// policy (config regenerated by an earlier runtime). Surface that on the same
// per-command update-notice channel so older-policy repos hear about it.
export function reviewPolicyUpdateNotice(repoRoot: string): string | null {
  try {
    const config = loadWorkflowConfig(repoRoot);
    const configured = config.reviewGates?.policyVersion ?? LEGACY_REVIEW_GATES_POLICY_VERSION;
...
function maybeNotifyReviewPolicy(repoRoot: string): void {
  const notice = reviewPolicyUpdateNotice(repoRoot);
  if (notice) {
...
export function maybeNotifyUpdate(cwd: string): AutoUpdateResult {
  ...
  maybeNotifyReviewPolicy(repoRoot);
...
export async function runUpdate(cwd: string, options: UpdateOptions): Promise<UpdateResult> {
  ...
  if (options.check || status.upToDate) {
    ...
    const policyNotice = reviewPolicyUpdateNotice(repoRoot);
    if (policyNotice) {
      writeUpdateOutput(options, policyNotice);
    }
